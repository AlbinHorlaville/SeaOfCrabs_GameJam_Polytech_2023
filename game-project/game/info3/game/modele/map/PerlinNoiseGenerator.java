...
public class PerlinNoiseGenerator {
	 ...
	 * consecutive number
	 */
	private final static int perlinTable[] = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225,
			...
			78, 66, 215, 61, 156, 180, 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36,
	...
	public double[][] generateNoiseArray(int width, int height, double noiseStartX, double noiseStartY) {
		...
		double startY = noiseStartY;

		for (int i = 0; i < width; i++) {
			startX = noiseStartX;
			for (int j = 0; j < height; j++) {
				startX += this.offset;
				noiseArray[i][j] = perlinNoise2d(startX, startY);
			}
			startY += this.offset;
		}
